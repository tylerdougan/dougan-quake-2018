...
with open('blast-results-2017-1.txt') as blastOutput:
	for j, line in enumerate(blastOutput):
		...
		if genesToIndicies[separatedLine[0][4:]] != genesToIndicies[separatedLine[1]]:
...
sio.savemat('blast-distance-2017-3.mat', {'inverseMatrix':inverseDistanceMatrix})



t1 = Sys.time()
summary(lm(matFile$inverseMatrix[429,] ~ matFile$inverseMatrix[99,]))
t2 = Sys.time()
t2 - t1
